...
evalExpr_ k e0 locs strms = case e0 of
  ...
  ExternMatrix _ _ name rows cols idxr idxc xs _ -> evalMatrix k name evalIdxr evalIdxc xs rows cols
    where evalIdxr = evalExpr_ k idxr locs strms
          evalIdxc = evalExpr_ k idxc locs strms
...
evalMatrix k name idxr idxc exts rows cols =
  case exts of
    ...
    Just xs ->
      case safeIndex k xs of
        Nothing  -> throw (NotEnoughValues name k)
        Just matr -> if   length (take rows matr) == rows
                       && length (take (rows+1) matr) == rows
                      then case safeIndex (fromIntegral idxr) matr of
                             Nothing -> throw (MatrixRowIdxOutofBounds name (fromIntegral idxr) rows)
                             Just row -> if  length (take cols row) == cols
                                          && length (take (cols+1) row) == cols
...
--safeMatrixIndex :: Int -> Int -> [[a]] -> Maybe a
--safeMatrixIndex idxr idxc m =
--   case safeIndex idxr m of
--      Just row -> safeIndex idxc row
--      Nothing -> Nothing
